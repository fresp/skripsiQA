...
import { UserData } from './user-pm.model';
import global from "../../global";

@Component({
  selector: 'app-user-pm-read',
  templateUrl: './user-pm-read.component.html',
  styleUrls: ['./user-pm-read.component.scss']
})
export class UserPmReadComponent implements OnInit {
  ...
  getList(){
    ...
    this.userPmService.listProjectManager(data).then(restData => {
...
// end pagination
  ...
  goToInsert(){
    this.router.navigate(['../projectManager-insert'], {relativeTo: this.route});
  }

  goToEdit(user_id: string){
    this.router.navigate(["../projectManager-edit", user_id], {relativeTo: this.route});
  }
  ...
  openDialogDelete(user_id: string){ //, name: string
    ...
    Swal.fire({
      ...
      if (result.value) {
        ...
        this.userPmService.deleteProjectManager(data).then(restData => {
          console.log(restData);
          if (restData.code == 200) {
            Swal.fire(
              'Deleted!',
  ...
  openDialogImport(){
    Swal.fire({
      ...
      preConfirm: (upload) => {
        ...
        return this.userPmService.importProjectManager(data).then(restData => {
          if (restData.code == 200) {
            console.log(restData);            
          }
        }).catch( () => {
