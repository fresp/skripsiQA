...
@Component({
  selector: 'app-user-pm-edit',
  templateUrl: './user-pm-edit.component.html',
  styleUrls: ['./user-pm-edit.component.scss']
})
export class UserPmEditComponent implements OnInit {
  ...
  detailUser(){
    ...
    this.userPmService.detailProjectManager(data).then(restData => {
  ...
  onSubmit(form: NgForm){
    ...
    if (this.json_locstor !== null) {
      if (form.status == "VALID" && this.validateImage) {
        ...
        this.userPmService.editProjectManager(this.projectManager_edit).then(restData => {
