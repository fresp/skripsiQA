...
export class BoardReadComponent implements OnInit {
  ...
  displayedColumns: string[] = ['action', 'code', 'title', 'status', 'created_date'];
...
// end pagination
  ...
  goToInsert(){
    this.router.navigate(['../board-insert'], {relativeTo: this.route});
  }

  goToEdit(user_id: string){
    this.router.navigate(["../board-edit", user_id], {relativeTo: this.route});
  }

  goToTicket(user_id: string){
    this.router.navigate(["../board/", user_id], {relativeTo: this.route});
  }
  ...
  openDialogDelete(user_id: string){ //, name: string
    ...
    Swal.fire({
      ...
      if (result.value) {
        ...
        this.boardService.deleteData(data).then(restData => {
          console.log(restData);
          if (restData.code == 200) {
            Swal.fire(
              'Deleted!',
  ...
  openDialogImport(){
    Swal.fire({
      ...
      preConfirm: (upload) => {
        ...
        return this.boardService.importData(data).then(restData => {
          if (restData.code == 200) {
            console.log(restData);            
          }
        }).catch( () => {
